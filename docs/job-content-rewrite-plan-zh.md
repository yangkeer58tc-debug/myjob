...
| **对 SEO 的底线** | 单页正文去空白后建议 **≥ 800 字符**（约 **120+** 西班牙语词，含列表）；过短易被判定薄内容。程序默认阈值可用 **800**；若 `raw_text` 极短，允许降到 **≥ 450 字符**但须在流水线打标 `short_source`。全站禁止共用同一长段 boilerplate（见 十·反模式） |
...
- `body_markdown` 去空白后长度 ≥ **800** 字符（默认）；若输入标记 `short_source: true` 则 ≥ **450** 字符。  
...
- `short_source`：当 `raw_text` 极短（如少于 400 字符）时由调用方置 `true`，与 **第四节·步骤 C**、**第八节 Prompt** 长度规则联动。  
...
8) Length: normally aim for at least ~800 characters total in body_markdown (excluding markdown syntax). If the input JSON has `"short_source": true` because raw_text is under 400 characters, aim for at least ~450 characters. If raw_text is longer, prefer ~120-250 Spanish words in Resumen + bullets combined—expand only by clarifying wording, never inventing facts.
...
- [ ] 五个 `**...**` 小节标题齐全且顺序正确  
- [ ] 长度 ≥ 调用方阈值（默认 800+ 字符，或 `short_source` 时 450+）  
...
| 2.0.1 | 2026-05 | 统一长度阈值：默认 ≥800 字符 / `short_source` 时 ≥450；输入 JSON 增加 `short_source` |
