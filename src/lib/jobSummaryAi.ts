...
const buildUserPrompt = (description: string) => {
  ...
  return [
    ...
    '',
    'Language: This job board targets Mexico. Write BOTH "summary" and every "highlights" entry in **Mexican Spanish (es-MX)**.',
    '- If the JD is already in Spanish (any region), adapt wording naturally to es-MX when needed.',
    '- If the JD is in English or another language, translate faithfully into es-MX without adding facts.',
    '',
    'Field "summary" (optional): one-sentence job summary.',
    '- Must be in Mexican Spanish (es-MX) as above.',
      ...
      messages: [
        {
          role: 'system',
          content:
            'You extract recruiting metadata from job descriptions. Always return valid JSON with keys summary (string or null) and highlights (array of strings). All user-visible strings must be Mexican Spanish (es-MX).',
        },
